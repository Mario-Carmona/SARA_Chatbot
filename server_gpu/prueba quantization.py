...
with torch.no_grad():
    ...
    ds_engine = deepspeed.initialize(model=model, dtype=torch.int8, config_params=ds_config)[0]
...
with torch.no_grad():
    outputs = ds_engine.module.generate(inputs, do_sample = True, max_new_tokens = 100, synced_gpus=True)
