...
class JoinDatasetsArguments(ProyectArguments):
    ...
    def __post_init__(self):
        ...
        for dataset_path in self.list_datasets:
            ...
            assert self.dataset_path.aplit('.')[-1] == 'csv', "`list_datasets` debe contener archivos CSV"

        dir = '/'.join(self.join_dataset_file.aplit('/')[:-1])
        assert os.path.exists(dir), "`join_dataset_file` debe estar en un directorio existente."
        assert self.join_dataset_file.aplit('.')[-1] == 'csv', "`join_dataset_file` debe ser un archivo CSV"
